...
export default function App() {
  ...
  return (
    <div className="min-h-screen text-neutral-300" style={{ background: theme.bg }}>
      ...
      >
        <div className="relative mx-auto flex max-w-6xl items-center px-4 py-8">
          <Link to="/" className="text-2xl font-bold tracking-wide text-neutral-100">
            二游小游戏合集
          </Link>
          <nav className="absolute left-1/2 flex -translate-x-1/2 items-center gap-1">
            {NAV_MENU.map((g) => (
              <div key={g.game} className="group relative">
                <button
                  title={g.game}
                  className="flex items-center rounded-md border border-transparent p-4 transition-colors group-hover:border-neutral-700"
                  style={{ background: 'transparent' }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = theme.panel)}
                  onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
                >
                  <img src={g.logo} alt={g.game} className="h-12 w-auto rounded object-contain" />
                </button>
                {/* 下拉菜单（pt-1 桥接悬停间隙） */}
                <div className="invisible absolute left-0 top-full pt-1 opacity-0 transition-opacity group-hover:visible group-hover:opacity-100">
                  ...
                  >
                    <div className="border-b border-neutral-800 px-4 py-2 text-[11px] tracking-wider text-neutral-500">
                      {g.game}
                    </div>
                    {g.items.map((it) => (
                      <Link
